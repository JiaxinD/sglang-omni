    ...
    *,
    ...
    async def quality(results):
        ...
        for result in results:
            ...
            verdicts[result.request_id] = detail.is_success and detail.wer <= max_wer
            details[result.request_id] = {
                **asdict(detail),
                "quality_pass": verdicts[result.request_id],
            }
        (destination / "quality-detail.json").write_text(
            json.dumps(
                {"lang": lang, "max_wer": max_wer, "requests": details},
                indent=2,
                ensure_ascii=False,
                allow_nan=False,
            ),
