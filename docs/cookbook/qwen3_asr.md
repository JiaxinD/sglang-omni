...
next GPU decode forward even for a single request. Use `--asr.scheduler.enable_async_decode false` to
disable it, or tune the crossover with `--asr.scheduler.async_decode_min_batch_size`.
...
sgl-omni serve \
  ...
  --asr.scheduler.enable_async_decode false \
  --port 8000
```
