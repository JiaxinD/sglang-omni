...
def _one(url, s):
    payload = {
        ...
        "max_new_tokens": MNT,
        "temperature": 0.0,
    }  # greedy -> deterministic, fixed length
    t0 = time.time()
...
def _bench_config(gpu, port, conc, async_on, samples):
    ...
    print(f"\n=== bs={conc} async={tag} (GPU{gpu}, greedy, olen={MNT}) ===", flush=True)
    ...
    try:
        ...
        for r in range(RUNS):
            ...
            print(
                f"  run {r:2d}: mean_lat={m:7.1f}ms thrpt={tp:5.2f}req/s{warm}",
                flush=True,
            )
            if r > 0:
                lat_means.append(m)
...
def main():
    ...
    for conc in (1, 4):
        ...
        print(
            f"bs={conc} OFF-ON latency delta = {d:+.1f}ms ({dpct:+.1f}%), "
            f"95%CI[{cipct[0]:+.1f}%,{cipct[1]:+.1f}%] p={p:.4f} -> {sig}",
            flush=True,
        )
        out["configs"][f"bs{conc}_delta"] = {
            "lat_delta_ms": d,
            "lat_delta_pct": dpct,
            "lat_delta_ci95_pct": list(cipct),
            "welch_dof": dof,
            "p_value": p,
