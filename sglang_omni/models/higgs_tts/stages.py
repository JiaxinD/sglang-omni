...
def create_vocoder_executor(
    ...
    *,
    ...
    if compile_decode:
        ...
        # Note: (Jiaxin Deng) capture the streaming decode windows the scheduler can
        # emit, [1, max(stride, followup + holdback + overlap)] at batch 1. Wider
        # windows (a batched final decode, a fast-first-audio catch-up) stay eager,
        # so the emitted audio is the same either way.
        max_window_frames = max(
            stream_stride,
            stream_followup_stride + stream_holdback_tokens + stream_overlap_tokens,
        )
        codec.warmup_cuda_graph(
            [(1, frames) for frames in range(1, max_window_frames + 1)]
        )
